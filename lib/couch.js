 ...
 *
 */
exports.getDust = function (app, user, dust, cb) {

   var appDB = nano.use(user + "_" + app);

   appDB.get(dust, function (err, res) {
      if (err) {
         console.log(err);
         cb(err.error, null);
      }
...
exports.createUser = function (creds, cb) {
   ...
   var user = {
      _id: "org.couchdb.user:" + creds.user,
      name: creds.user,
      roles: [],
      type: "user",
      password: creds.pass,
